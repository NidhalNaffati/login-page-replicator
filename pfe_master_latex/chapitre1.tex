...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.82\textwidth,keepaspectratio]{images/sopra-over-view}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.82\textwidth,keepaspectratio]{images/sopra-hr-org-chart}
    \caption{Organigramme de Sopra HR Software}
    \label{fig:sopra_hr_org_chart}
\end{figure}

Notre stage de fin d'études a été réalisé au sein de l'\textbf{équipe R\&D (Research \&
Development)}. Cette équipe a pour mission de concevoir, développer et améliorer les solutions
logicielles RH de Sopra HR (notamment HR Access et Pleiades), en menant des travaux de
recherche appliquée et en intégrant de nouvelles technologies dans les produits existants.

Au sein de cette équipe R\&D, une cellule dédiée à la \textbf{qualité logicielle et à
l'automatisation des tests} s'appuie sur un framework interne de tests BDD
(\textit{Behavior-Driven Development}) nommé \textbf{T4T} (\textit{Test for Teams}).
Ce framework permet aux ingénieurs de rédiger des scénarios de test en langage naturel
structuré (syntaxe Gherkin~: \texttt{Given/When/Then}) qui sont ensuite exécutés
automatiquement contre les applications cibles. T4T est l'outil central de la stratégie
...
Dans ce contexte d'équipe R\&D, la mission confiée pour ce projet de fin d'études est
...
aux projets réels de l'équipe R\&D utilisant T4T.
...
\begin{infobox}
    ...
    existante de l'équipe R\&D, en intégrant la sécurité à chaque étape selon une approche
...
L'équipe R\&D exécutait ses \textbf{tests BDD} rédigés avec le framework interne

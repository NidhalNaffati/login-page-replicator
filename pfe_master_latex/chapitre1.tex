...
Notre stage de fin d'études a été réalisé au sein de l'équipe internationale R\&D. Cette
équipe a pour mission de rechercher, de développer et de proposer des solutions répondant
à l'évolution des besoins des directeurs des ressources humaines, tout en anticipant les
tendances et perspectives futures du domaine RH.
...
\begin{infobox}
    \textit{Comment transformer un pipeline CI/CD basique (Jenkins + tests unitaires) en une
    chaîne DevSecOps complète qui intègre la sécurité à chaque étape, sans sacrifier la vélocité
    de livraison, et avec des preuves auditables de conformité ?}
\end{infobox}
...
manuelle.
Les jobs Jenkins étaient définis et administrés à la main par les développeurs et
les builds étaient souvent lancés manuellement par un développeur plutôt que par des
déclencheurs automatisés.
Jenkins était relié à notre dépôt Git, mais l'intégration restait
limitée: il se contentait d'exécuter des tests unitaires avant un déploiement
manuel effectué via des scripts shell.
...
Tests & Uniquement tests BDD \\ \hdashline
...
Le pipeline Jenkins se contentait de vérifier que les tests passaient. Aucune analyse statique
de sécurité (SAST), aucun scan de dépendances (SCA), aucun scan d'images Docker n'était effectué.
Des vulnérabilités critiques pouvaient transiter du code source jusqu'en production sans être
détectées.
...
Les tests unitaires ne testaient que des fonctions isolées, sans jamais valider le comportement
réel de l'application dans son contexte de déploiement.
